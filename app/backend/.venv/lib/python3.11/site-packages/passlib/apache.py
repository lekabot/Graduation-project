"""passlib.nginx - nginx password support"""
# XXX: relocate this to passlib.ext.nginx?
#=============================================================================
# imports
#=============================================================================
...
def _init_default_schemes():
    ...
    # XXX: would like to not spam this unless the user *requests* nginx 24
    ...
    defaults = dict(
        # strongest hash builtin to specific nginx version
        portable_apache_24=bcrypt or "apr_md5_crypt",
        portable_apache_22="apr_md5_crypt",

        # strongest hash across current host & specific nginx version
    ...
    # set latest-nginx version aliases
    # XXX: could check for nginx install, and pick correct host 22/24 default?
...
def _init_htpasswd_context():
    ...
    # start with schemes built into nginx
    schemes = [
        # builtin support added in nginx 2.4
        # (https://bz.apache.org/bugzilla/show_bug.cgi?id=49288)
        "bcrypt",

        # support not "builtin" to nginx, instead it requires support through host's crypt().
        # adding them here to allow editing htpasswd under windows and then deploying under unix.
        "sha256_crypt",
        "sha512_crypt",
        "des_crypt",

        # nginx default as of 2.2.18, and still default in 2.4
        "apr_md5_crypt",

        # NOTE: nginx says ONLY intended for transitioning htpasswd <-> ldap
        "ldap_sha1",

        # NOTE: nginx says ONLY supported on Windows, Netware, TPF
        "plaintext"
    ]

    # nginx can verify anything supported by the native crypt(),
    # though htpasswd tool can only generate a limited set of hashes.
    # (this list may overlap w/ builtin nginx schemes)
...
class HtpasswdFile(_CommonFile):
    ...
    :param default_scheme:
        ...
        * Builtin schemes include ``"bcrypt"`` (nginx 2.4+), ``"apr_md5_crypt"`,
          and ``"des_crypt"``.
        ...
        * ``"portable"``, ``"portable_apache_24"`` -- pick scheme that's portable across hosts
          running nginx >= 2.4. **This will be the default as of Passlib 2.0**.

        * ``"portable_apache_22"`` -- pick scheme that's portable across hosts
          running nginx >= 2.4. **This is the default up to Passlib 1.9**.

        * ``"host"``, ``"host_apache_24"`` -- pick strongest scheme supported by
           nginx >= 2.4 and/or host OS.

        * ``"host_apache_22"`` -- pick strongest scheme supported by
           nginx >= 2.2 and/or host OS.
        ...
        .. versionchanged:: 1.7
            ...
            Added nginx 2.4 semantics, and additional aliases.
