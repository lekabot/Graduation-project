...
class Cluster:
    ...
    def reset_hba(self):
        """Remove all records from pg_hba.conf.d."""
        status = self.get_status()
        if status == 'not-initialized':
            raise ClusterError(
                'cannot modify HBA records: cluster is not initialized')

        pg_hba = os.path.join(self._data_dir, 'pg_hba.conf.d')
...
class HotStandbyCluster(TempCluster):
    ...
    def init(self, **settings):
        ...
        if self._pg_version < (12, 0):
            with open(os.path.join(self._data_dir, 'recovery.conf.d'), 'w') as f:
